...
// ============ MAX30102 VARIABLES (Heart Rate & SpO2) ============
...
// ============ MPU6050 VARIABLES (Steps & Sleep) ============
unsigned long stepCount = 0;
float lastAccelMagnitude = 0;
bool stepDetected = false;
unsigned long lastStepTime = 0;
const unsigned long MIN_STEP_INTERVAL = 200;
const float STEP_THRESHOLD = 0.8;              // LOWER threshold = MORE SENSITIVE (was 1.3)
...
const unsigned long SLEEP_START_DELAY = 5000;  // 5 SECONDS (was 15000)
...
// ============ MLX90614 VARIABLES (Temperature) ============
...
void setup() {
  ...
  Serial.println("═══════════════════════════════════════");
  Serial.println("MAX30102 + MPU6050 + MLX90614 + BLE");
  ...
  Serial.println("🚶 Walk to count steps (HIGH SENSITIVITY)");
...
void sendAllSensorData() {
  ...
  if (isnan(validTemp) || validTemp < -40 || validTemp > 100) {
    validTemp = 0.0;  // Default to 0 if invalid
  }
